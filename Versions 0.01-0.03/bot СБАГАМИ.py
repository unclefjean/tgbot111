...
async def document_handler(message: Message, state: FSMContext):
    ...
    if len(pdf_files) >= 2:  # Объединяем, если есть минимум 2 файла
...
def cleanup_temp_folder(folder="downloads"):
    """
    Удаляет все файлы в указанной папке и саму папку, если она пуста.
    """
    if os.path.exists(folder):
        ...
        try:
            os.rmdir(folder)  # Удаляем папку, если она пуста
            logger.info(f"Папка {folder} удалена.")
        except Exception as e:
            logger.warning(f"Ошибка при удалении папки {folder}: {e}")
