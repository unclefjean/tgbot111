...
def arrange_pdfs_side_by_side(pdf_files, rows, cols, output_file):
    """Размещает страницы нескольких PDF файлов на одном листе"""
    ...
    for pdf_file in pdf_files:
        images = pdf_to_images(pdf_file)  # Получаем изображения из PDF
        for img_path in images:
            row = img_index // cols
            col = img_index % cols
            x_pos = col * cell_width
            y_pos = page_height - (row + 2) * cell_height

            # Размещаем изображение на листе
            c.drawImage(img_path, x_pos, y_pos, width=cell_width, height=cell_height)

            # Если достигнут предел строк, создаем новую страницу
            img_index += 1
...
async def document_handler(message: Message, state: FSMContext):
    ...
    # Дублируем файл для выполнения выбранного макета
    pdf_files = [file_path, file_path]  # Один и тот же файл используется несколько раз
    await state.update_data(pdf_files=pdf_files)

    # Генерируем объединённый файл
    merged_file = merge_pdfs(pdf_files, layout)

    # Отправляем результат пользователю и показываем кнопки
